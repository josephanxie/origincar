...
class ResNetControlNode(Node):
    def __init__(self):
        ...
        # 导入ResNet18模型（绝对路径）
        self.models = [dnn.load('/root/follow2.bin'), dnn.load('/root/ob4.bin')]
    ...
    def follow_line(self, img):
        costmap = cv2.distanceTransform(
            cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), cv2.DIST_L2, 3
        ) 
        costmap_roi = costmap[160:384,]
        ...
        image_msg_with_point = self.bridge.cv2_to_imgmsg(img, "bgr8")
