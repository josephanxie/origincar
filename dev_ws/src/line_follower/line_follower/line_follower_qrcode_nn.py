...
class ResNetControlNode(Node):
    def __init__(self):
        ...
        self.follow_sub = self.create_subscription(Image, '/image_fox', self.follow_line, 10)
        ...
        self.control_sub = self.create_subscription(Int32, '/control', self.receive, 1)
    ...
    def qrcode_(self):
        if not (not self.first_run and self.aa_run):
            return
        import sys
        if sys.argv[1] == "0":
            self.qr_result = 3
    ...
    def receive(self, msg):
        if msg.data == 0:
            self.run = True
            self.create_timer(5.5, self.qrcode_) 
