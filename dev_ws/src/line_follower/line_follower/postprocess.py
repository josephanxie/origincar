...
def postprocess(model_output,
    ...
    pred_sbbox = model_output[2].buffer.reshape([1, 84, 84, 3,
                                                      6]).transpose([0, 3, 1, 2, 4])
    pred_mbbox = model_output[1].buffer.reshape([1, 42, 42, 3,
                                                      6]).transpose([0, 3, 1, 2, 4])
    pred_lbbox = model_output[0].buffer. reshape([1, 21, 21, 3,
                                                     6]).transpose([0, 3, 1, 2, 4])
...
def draw_bboxs(image, bboxes, gt_classes_index=None, classes={0: "construction"}):
