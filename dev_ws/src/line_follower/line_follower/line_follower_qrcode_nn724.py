...
class ResNetControlNode(Node):
    def __init__(self):
        ...
        self.follow_sub = self.create_subscription(Image, '/image', self.follow_line, 10)
        self.qrcode_sub = self.create_subscription(Image, '/image', self.qrcode, 10)
        ...
        self.models = [dnn.load('/root/follow723.bin')]
    ...
    def follow_line(self, msg):
        ...
        # return
        self.Error = 115 - 1.0 * x / 640 * 224  # 计算偏差
    ...
    def avoid_obstraction(self, msg):
        ...
        for num, target in enumerate(msg.targets):
            ...
            if target.rois[0].confidence > 0.6:
                ...
                y = target.rois[0].rect.y_offset + target.rois[0].rect.height // 2
                S = target.rois[0].rect.height * target.rois[0].rect.width #TODO
                if S > max_target[2]:
                    max_object = [x, y, S]
                
        self.get_logger().info(f"最大物体位置({x}, {y}), 面积为{S}")
        # return # return 结束
        # 721注释 722
        if S > 30000:
            self.stop()
